...
class CephService(object):
    ...
    def get_encryption_config(cls, daemon_name: str):
        ...
        if EncryptionTypes.KMS.value:
            ...
            vault_config = VaultConfig(**vault_config_data)
            kmip_config = KmipConfig(**kmip_config_data)

            encryption_config = encryption_config._replace(
                kms=KmsConfig(vault=vault_config, kmip=kmip_config)
            )
    ...
    @classmethod
    def set_encryption_config(cls, encryption_type, kms_provider, config: Union[VaultConfig, rgw.KmipConfig], daemon_name):
        ...
        def _generate_keys(encryption_type: str, backend: str, config_name: str):
        ...
        for field in config._fields:
            ...
            key = _generate_keys(encryption_type, kms_provider, field)
...
def _generate_key(encryption_type, backend, config_name):
...
def _get_conf_keys(fields: List[str], req_fields: List[str], encryption_type: str, backend: str, daemon_name: str) -> Dict[str, str]:
    ...
    for field in fields:
        ...
        try:
            ...
            logger.error("Error fetching Vault config for key %s: %s", key, str(e))
