...
import { MirroringPathUtils, FS_ROOT, FS_ROOT_PATH_SENTINEL } from '../mirroring-path-utils';
...
export class MirroringPathsStepComponent implements OnInit, TearsheetStep {
    ...
    this.cephfsService
      ...
      .subscribe((dirs) => {
        ...
        const options = dirs
          ...
          .filter(
            (name) =>
              !MirroringPathUtils.isPathTracked(
        ...
        if (
          parentPath === FS_ROOT &&
          levelIndex === 0 &&
          !MirroringPathUtils.conflictsWithMirroredPath(FS_ROOT, this.trackedPaths) &&
          !this.overlapsOtherSelectedPath(FS_ROOT, pathIndex)
        ) {
  ...
  private isPathSelectable(path: string, pathIndex: number): boolean {
    const normalized = MirroringPathUtils.normalizePath(path);
    if (!normalized || MirroringPathUtils.conflictsWithMirroredPath(normalized, this.trackedPaths)) {
      return false;
    }

    return !this.overlapsOtherSelectedPath(normalized, pathIndex);
  }

  private overlapsOtherSelectedPath(path: string, pathIndex: number): boolean {
    return this.paths.some((entry, index) => {
      if (index === pathIndex) {
        return false;
      }
      const selected = MirroringPathUtils.normalizePath(entry.fullPath);
      return selected && MirroringPathUtils.pathsOverlap(path, selected);
    });
  }
