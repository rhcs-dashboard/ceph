...
export class NvmeofGroupFormComponent extends CdForm implements OnInit {
  ...
  createForm() {
    this.groupForm = new CdFormGroup({
      groupName: new UntypedFormControl(null, {
        ...
        asyncValidators: [CdValidators.unique(this.nvmeofService.exists, this.nvmeofService)]
      }),
  ...
  onHostSelectionChange(selection: any): void {
    this.hasSelectedHosts = selection.selected?.length > 0;
    if (this.hasSelectedHosts) {
      this.showHostSelectionError = false;
    }
  }
  ...
  onSubmit() {
    ...
    if (selectedHostnames.length === 0) {
      this.showHostSelectionError = true;
      this.groupForm.setErrors({ cdSubmitButton: true });
      return;
    }
    this.showHostSelectionError = false;
