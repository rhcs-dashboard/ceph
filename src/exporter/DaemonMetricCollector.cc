...
void DaemonMetricCollector::dump_asok_metrics() {
  ...
  for (auto &[daemon_name, sock_client] : clients) {
    ...
    for (auto &perf_group_item : counter_schema) {
      ...
      json_array perf_grp_dump_array = counter_dump[perf_group].as_array();
      for (auto schema_itr = perf_group_schema_array.begin(), dump_itr = perf_grp_dump_array.begin(); schema_itr != perf_group_schema_array.end() && dump_itr != perf_grp_dump_array.end(); ++schema_itr, ++dump_itr) {
        auto& perf_grp = *schema_itr;
        auto& perf_dump_grp = *dump_itr;
        auto counters = perf_grp.at("counters").as_object();

      for (auto &perf_grp : perf_group_schema_array) {
        auto counters_labels = perf_grp.at("labels").as_object();
        auto counters_values = perf_dump_grp.at("counters").as_object();
        labels_t labels;
        ...
        for (auto &counter : counters) {
          ...
          std::string counter_name = perf_group + counter_name_init;
