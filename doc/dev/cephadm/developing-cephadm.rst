...
Docker inside Docker. This approachs his its downsides too as we have to
...
In order to setup Cephadm's box run::
        ...
        sudo ./bootstrap.sh --update-ceph-image --update-box-image --skip-bootstrap

After getting all needed images we can run::

        sudo ./bootstrap.sh --osds 3 --hosts 3

or if you want to do it all in one command::

        sudo ./bootstrap.sh --update-ceph-image --update-box-image --osds 3 --hosts 3
...
After boostraping the cluster you can go inside the seed box in which you'll be
able to run cehpadm's commands::
...
If you want to add hosts and don't want to deal with finding the hostname and
host ip on your own, run::
